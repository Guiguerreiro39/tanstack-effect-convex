...
import { Email, Name, Password } from "../model/schemas";
...
export function SignUpForm() {
  ...
  const handleSubmit = async () => {
    setError(null);
    const nameResult = Schema.decodeUnknownEither(Name)(name);
  ...
  return (
    <View className="mx-auto w-full max-w-sm space-y-4 p-4">
      ...
      {error ? (
        <View className="relative z-10 rounded-md bg-red-100 p-3">
          <Text className="font-medium text-red-700 text-sm">{error}</Text>
        </View>
      ) : null}

      <View className="space-y-1">
        <Text className="font-medium text-gray-700 text-sm">Name</Text>
        <TextInput
